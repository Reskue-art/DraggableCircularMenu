...
function createBreakpoints(breakpoints) {
  const {
    ...
    other = _objectWithoutPropertiesLoose(breakpoints, _excluded$9);
...
var useDraggable = function (_a) {
    var initialAngle = _a.initialAngle;
    var _b = React__default.useState(), node = _b[0], setNode = _b[1];
    var _c = React__default.useState(initialAngle), angle = _c[0], setAngle = _c[1];
    var _d = React__default.useState({
        dx: 0,
        dy: 0,
    }), _e = _d[0], dx = _e.dx, dy = _e.dy, setOffset = _d[1];
    ...
    React__default.useEffect(function () {
        ...
        var containerWidth = (_a = node.parentElement) === null || _a === void 0 ? void 0 : _a.getBoundingClientRect().width;
        var radius = containerWidth ? containerWidth / 2 : 0;
        var center = radius - width / 2;
        var radian = ((angle + 0.25) % 1) * Math.PI * 2 - Math.PI;
        var dx = center + radius * Math.cos(radian);
        var dy = center + radius * Math.sin(radian);
    ...
    var handleMouseDown = React__default.useCallback(function (e) {
        ...
        var handleMouseMove = function (e) {
            ...
            var centerDistance = Math.sqrt(Math.pow(dx - center, 2) + Math.pow(dy - center, 2));
            var sinValue = (dy - center) / centerDistance;
            var cosValue = (dx - center) / centerDistance;
            dx = center + radius * cosValue;
            dy = center + radius * sinValue;
            var radians = Math.atan2(dy - center, dx - center);
            var angle = (radians + Math.PI) / (Math.PI * 2);
    ...
    var handleTouchStart = React__default.useCallback(function (e) {
        var _a;
        if (!node) {
            return;
        }
...
*/
var defaultData = Array.from({ length: 101 }, function (_, i) { return i.toString(); }); // Generates an array from 0 to 100
/*
|--------------------------------------------------------------------------
| Utils
|--------------------------------------------------------------------------
...
|--------------------------------------------------------------------------
*/
var RadialProgressBar = styled(Box$1)(function () { return ({
    background: "#D9D9D9",
    position: "relative",
    borderRadius: "50%",
    overflow: "hidden",
...
var RPBHalf1 = styled(Box$1)(function (_a) {
    var circleColor = _a.circleColor;
    return ({
        height: "50%",
        width: "100%",
...
var BoxDraggable = styled(Box$1)(function (_a) {
    var circleColor = _a.circleColor;
    return ({
        background: circleColor,
        borderRadius: "50%",
...
var TypographyStyled = styled(Typography$1)(function () { return ({
    textAlign: "center",
    fontFamily: "Anton",
    fontSize: "1.25rem",
}); });
 ...
 */
var DraggableCircularMenu = function (_a) {
    var _b = _a.values, values = _b === void 0 ? defaultData : _b, // Uses defaultData if values is not provided
    _c = _a.circleColor, // Uses defaultData if values is not provided
    circleColor = _c === void 0 ? "#00c8b7" : _c, _d = _a.textColor, textColor = _d === void 0 ? 'black' : _d, 
    // options = [],
    _e = _a.text, 
    // options = [],
    text = _e === void 0 ? "" : _e;
    var _f = React__default.useState(0.75), valueProgress = _f[0]; _f[1];
    // const [selectedCurrencyIndex, setSelectedCurrencyIndex] = React.useState(0);
    var _g = useDraggable({
        initialAngle: valueProgress,
    }), draggbleRef = _g[0], dx = _g[1], dy = _g[2], angle = _g[3];
    // Render
    //--------------------------------------------------------------------------
    return (React__default.createElement(Box$1, { component: "div" },
        React__default.createElement(RadialProgressBar, null,
            React__default.createElement(RPBHalf1, { circleColor: circleColor }),
            React__default.createElement(RPBHalf2, { style: {
                    background: angle > 0.5 ? circleColor : "inherit",
                    transform: "rotate(".concat(angle > 0.5
                        ? ((angle - 0.25) % 1) * 360
                        : ((angle + 0.25) % 1) * 360, "deg)"),
                } }),
            React__default.createElement(Overlay, null),
            React__default.createElement(Circle, null,
                React__default.createElement(BoxDraggable, { circleColor: circleColor, ref: draggbleRef, style: {
                        transform: "translate(".concat(dx, "px, ").concat(dy, "px)"),
                        zIndex: 9999,
                    } }),
                React__default.createElement(TypographyStyled, { sx: { color: textColor } },
                    React__default.createElement("span", { style: { fontSize: "2.5rem" } },
                        values[Math.round(angle * (values.length - 1))],
                        ".00",
                        " " + text),
                    React__default.createElement("br", null))))));
};

export { DraggableCircularMenu as default };
